...
from .models import Product, Season, Category
...
def hot_products(request):
    ...
    seasonal = Product.objects.filter(season_sales__exact=True)
    high_rating = Product.objects.filter(rating__gt=4)
    discounted = Product.objects.all()
    season_fields = Season.objects.all()
